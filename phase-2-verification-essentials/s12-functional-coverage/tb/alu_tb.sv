...
module alu_tb();
    ...
    bind alu alu_coverage u_alu_cov (
        .clk   (alu_tb.clk),    // no clk in alu.sv
        .rst   (alu_tb.rst),    // no rst in alu.sv
        .opcode(opcode),
        .a     (a),
    ...
    alu_vec_t vectors [VECTOR_NUM]; // Vectors for W=32
    ...
    localparam N_CORNERS = 4;
    logic [W-1:0] corners [N_CORNERS] = '{
        32'h0000_0000,
        32'hFFFF_FFFF,
        32'h8000_0000,
        32'h0000_0001
    ...
    function automatic logic [W-1:0] pick_operand();
        if ($urandom_range(0, 99) < 40) return corners[$urandom_range(0, N_CORNERS-1)];
        else return $urandom();
    endfunction
    ...
    initial begin
        ...
        repeat (10000) begin
            @(posedge clk) #1;
            opcode = opcode_t'($urandom_range(ALU_ADD, ALU_SLTU));
            a = pick_operand();
            b = pick_operand();
        end
